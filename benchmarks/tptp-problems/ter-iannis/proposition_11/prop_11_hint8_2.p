% ./larus -vcoq -m12 -l100 benchmarks/tptp-problems/ter-iannis/proposition_11/prop_11_hint8_2.p
%%% Without hint
% No proof found
% With -i : None
...
fof(proposition_11_int,conjecture,(! [A,B,C] : (? [E,F] : ((betS(A,C,B)) => ((betS(A,C,E) & equilateral(A,E,F) & (C != F) )))))).

fof(hintname0, hint,_, 1, lemma_extension(0,2,0,2)).
fof(hintname2, hint,_, 2, proposition_01(0,?)).
fof(hintname3, hint, ((2 = 4) | (2 != 4)), _, _).
